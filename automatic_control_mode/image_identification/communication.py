...
class Communication():
    ...
    def pixel_to_angle(self,xy,z):
        ...
        y_0=665
        print("xy",xy)
        angle_x = str(round(math.atan( ( (pixel_x - pixel_center[0]) *pixel_ratio) / z0) *360/3.14*10)+x_0) # Convert the x pixel value into an angle value
        angle_y = str(round(math.atan( ( (pixel_y - pixel_center[1] ) *pixel_ratio) / z0) *360/3.14*10)+y_0) # Convert the y pixel value into an angle value
        posi_z=str(round(z))
