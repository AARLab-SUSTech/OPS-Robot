...
const float Z_PRE_MM   = 40.0;   // advanced at power-up, before any command
const float Z_EXTRA_MM = 20.0;   // added when the first coordinate arrives
const float Z_BACK_MM  = 60.0;   // retracted when sampling ends

boolean z_extended = false;      // true once Z_EXTRA_MM has been applied
...
void move(int x, int y, int z){
  ...
  // z == 0 means "sampling finished": retract and re-arm for the next run.
  if (z == 0)
  {
    z_move_mm(Z_PRE_MM + Z_EXTRA_MM - Z_BACK_MM);
    z_extended = false;
    Serial1.println("ok");
    return;
  }

  // First coordinate of a run: go the remaining way in before aiming.
  if (!z_extended)
  {
    z_move_mm(Z_PRE_MM + Z_EXTRA_MM);
    z_extended = true;
  }
